...
# Hardcoded crisis keywords – must be checked before anything else
CRISIS_KEYWORDS = [
...
async def route_message(phone: str, name: str, message: str,
    ...
    if message and _is_crisis_message(message):
        ...
        # Still log the event for admin attention
        try:
            from database.client import supabase
            supabase.table('crisis_events').insert({
                'phone_hash': phone,  # anonymised
    ...
    # 5. Media handling — before hard trigger check
...
async def _confirm_cancel(phone: str, student: dict, conversation: dict,
    ...
    if message.strip().upper() in ('YES CANCEL', 'YES', 'CONFIRM', 'CANCEL PLAN'):
        # Log the cancellation — admin handles actual Paystack subscription disable
        try:
            from database.client import supabase
...
Step 3 — Wire it into both handlers

In whatsapp/handler.py, find the command section (where trigger == 'PING' is). Add:
...
In telegram/handler.py, add the same block (change phone to str(chat_id) and 'whatsapp' to 'telegram').

---

Commit all three files.

Have you created the file and wired it? (yes/no)


So give me the complete code DO NOT cut anything in the complete code DO NOT cut my main code do not break anything give me the complete code to not change my logic and what do you think of the code the quality
