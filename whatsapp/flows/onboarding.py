...
Guides new students through account creation (10 steps).
Also handles existing student login (WAX ID + PIN).
...
async def handle_new_or_existing(phone: str, conversation: dict, message: str):
    """Handles the very first message from any user — new or returning."""
...
async def _step_new_or_existing(phone: str, conversation: dict, message: str, state: dict):
    ...
    if any(k in msg for k in new_keywords):
        ...
        await update_conversation_state(conversation['id'], 'whatsapp', phone, {
            'conversation_state': {'awaiting_response_for': 'name', 'onboarding_step': 1}
        })
...
async def _step_wax_id_entry(phone: str, conversation: dict, message: str, state: dict):
    ...
    from helpers import extract_wax_id, is_valid_wax_id

    # Try to extract WAX ID
    ...
    await send_whatsapp_message(
        ...
        f"Please enter your *4-digit PIN* to log in."
    )
...
async def _step_pin_entry(phone: str, conversation: dict, message: str, state: dict):
    ...
    if not verify_pin(pin, student['pin_hash']):
        ...
        if remaining == 0:
            await send_whatsapp_message(
                phone,
                "⚠️ *Account Locked* — Too many wrong attempts.\n\n"
...
async def _step_name(phone: str, conversation: dict, message: str, state: dict):
    ...
    first = name.split()[0]
    options = '\n'.join([f"{i+1}️⃣ {lvl}" for i, lvl in enumerate(CLASS_LEVELS)])
...
async def _step_target_exam(phone: str, conversation: dict, message: str, state: dict):
    ...
    exam_map = {
        ...
        '3': 'NECO', 'NECO': 'NECO',
        '4': 'COMMON_ENTRANCE', 'COMMON': 'COMMON_ENTRANCE', 'ENTRANCE': 'COMMON_ENTRANCE',
...
async def _step_subjects(phone: str, conversation: dict, message: str, state: dict):
    ...
    numbers = _re.findall(r'\d+', message)
    selected = []
...
async def _step_exam_date(phone: str, conversation: dict, message: str, state: dict):
    ...
    year_match = _re.search(r'20(2[4-9]|[3-9]\d)', msg)
    ...
    elif msg in ['not sure', 'soon', 'this year', 'idk', "don't know", 'unsure']:
    ...
    if not exam_date:
        await send_whatsapp_message(
            phone,
            "When is your exam? 📅\n\n"
            "Try:\n• *May 2025*\n• *June 2025*\n• *Not sure* (I'll plan around that!)"
        )
        return

    urgency = ""
    if days_left < 30:
        urgency = f"\n\n⚠️ Only {days_left} days left! We need to focus hard right away."
    elif days_left < 90:
...
async def _step_state(phone: str, conversation: dict, message: str, state: dict):
    ...
    await send_whatsapp_message(
        ...
        "1️⃣ *Standard English* — clear and formal\n"
        "2️⃣ *Naija Pidgin* — casual and relatable\n\n"
        "_(You can always switch this later)_"
    )
...
async def _step_language_pref(phone: str, conversation: dict, message: str, state: dict):
    ...
    await send_whatsapp_message(
        phone,
        f"*{lang_display}* 👍\n\n"
        "Almost done! Let's set your *security PIN*.\n\n"
        "Your PIN is a *4-digit number* you'll use to log in on any device.\n"
        "Keep it private — don't share it with anyone.\n\n"
        "What 4-digit PIN would you like? _(e.g., 5823)_"
    )
...
async def _step_pin_setup(phone: str, conversation: dict, message: str, state: dict):
    ...
    if not is_valid_pin(pin):
        await send_whatsapp_message(phone, "Your PIN must be *exactly 4 digits* 🔐\n\nPlease try again.")
        return
...
async def _step_pin_confirm(phone: str, conversation: dict, message: str, state: dict):
    ...
    try:
        ...
        welcome = (
            ...
            f"━━━━━━━━━━━━━━━━━━\n\n"
            f"⚠️ *Write these down somewhere safe.*\n"
            f"They're how you get back in if you lose your phone.\n\n"
            f"━━━━━━━━━━━━━━━━━━\n\n"
...
import re  # needed for WAX ID validation in _step_wax_id_entry
